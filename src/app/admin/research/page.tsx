...
import { FiPlus, FiEdit3, FiTrash2, FiFileText } from "react-icons/fi";
...
export default function AdminResearchPage() {
  ...
  return (
    <div>
            ...
            <div className="divide-y divide-gray-100">
              {items.map((item: any) => (
                <div key={item.id} className="flex items-center gap-3 sm:gap-4 p-4 sm:p-6 hover:bg-gray-50 transition-colors">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-accent-50 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {item.cover_image ? <img src={item.cover_image} alt="" className="w-full h-full object-cover rounded-xl" /> : <FiFileText className="w-5 h-5 sm:w-6 sm:h-6 text-accent-500" />}
                  </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "تعديل البحث" : "إضافة بحث جديد"} size="lg">
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                ...
                <input type="file" accept="image/*" onChange={handleUpload} disabled={uploading} className="mt-2 w-full text-sm text-gray-500 file:ml-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-accent-50 file:text-accent-600" />
                {uploading && <p className="text-xs text-accent-500 mt-1">جاري الرفع...</p>}
              </div>
              {imagePreview && (
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="featured" className="text-sm text-gray-700">بحث مميز</label>
          </div>
